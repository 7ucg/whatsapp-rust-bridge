...
impl super::Parameters for Parameters {
    ...
    fn generate<R: CryptoRng + ?Sized>(
        ...
        let mut seed = [0u8; 64]; // ML-KEM key-generation seed (d || z)
        csprng.fill_bytes(&mut seed);
        let (sk, pk) = kyber1024::generate_key_pair(seed).into_parts();
    ...
    fn encapsulate<R: CryptoRng + ?Sized>(
        ...
        let mut seed = [0u8; 32]; // ML-KEM encapsulation seed
        csprng.fill_bytes(&mut seed);
        let (kyber_ct, kyber_ss) = kyber1024::encapsulate(&kyber_pk, seed);
